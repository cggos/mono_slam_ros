...
public:
    ImageConverter(char *file = NULL) : it_(nh_), first(1), slam(file) {
    ...
    void imageCb(const sensor_msgs::ImageConstPtr &msg) {
        ...
        Mat frame;
        int scale = 1;
        cv::resize(cv_ptr->image, cv_ptr->image,
                   cv::Size(cv_ptr->image.size().width / scale, cv_ptr->image.size().height / scale));
